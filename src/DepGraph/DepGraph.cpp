...
static cl::opt<bool, false>
    includeAllInstsInDepGraph("includeAllInstsInDepGraph",
                              cl::desc("Include All Instructions In DepGraph."),
                              cl::NotHidden);
...
// Project: e-CoSoc
// Institution: Computer Science department of Federal University of Minas
// Gerais
//
//*********************************************************************************************************************************************************************

// FIXME: Deal properly with invoke instructions. An Invoke instruction can be
// treated as a call node
...
GraphNode::~GraphNode()
{
  for (std::map<GraphNode *, edgeType>::iterator pred = predecessors.begin();
  ...
  for (std::map<GraphNode *, edgeType>::iterator succ = successors.begin();
...
std::string llvm::GraphNode::getOriginalName()
{
  return originalName;
}
void llvm::GraphNode::setOriginalName(std::string name)
{
  this->originalName = name;
}

void llvm::GraphNode::print()
{
  if (this->getOriginalName() != "")
    errs() << "Node:" << this->getLabel()
...
std::string llvm::GraphNode::getColor()
{
...
OpNode::OpNode(int OpCode) : GraphNode(), OpCode(OpCode), value(NULL)
{
  this->Class_ID = 1;
  NrOpNodes++;
  // NrNodes++;
...
OpNode::OpNode(int OpCode, Value *v) : GraphNode(), OpCode(OpCode), value(v)
{
  this->Class_ID = 1;
  NrOpNodes++;
  // NrNodes++;
...
std::string llvm::OpNode::getColor()
{
...
std::string llvm::CallNode::getColor()
{
...
VarNode::VarNode(Value *value) : GraphNode(), value(value)
{
  this->Class_ID = 2;
  NrVarNodes++;
  // NrNodes++;
...
std::string llvm::VarNode::getColor()
{
  return std::string("white");
  // if (!isa<Constant> (value))
  //     return std::string("tan1");
  // else
  //     return std::string("white");
}

JoinNode::JoinNode(Value *value) : GraphNode(), value(value)
{
  this->Class_ID = 5;
  NrJoinNodes++;
  // NrNodes++;
 ...
 * Class MemNode
 */
MemNode::MemNode(int aliasSetID, AliasSets *AS) : aliasSetID(aliasSetID), AS(AS)
{
  this->Class_ID = 4;
  NrMemNodes++;
  // NrNodes++;
...
{
  return USE_ALIAS_SETS ? AS->getValueSets()[aliasSetID]
                        : std::set<llvm::Value *>();
}
...
Graph::Graph(AliasSets *AS) : AS(AS)
{
}
Graph::~Graph()
{
  for (std::set<GraphNode *>::iterator it = nodes.begin(); it != nodes.end();
       ++it)
  {
    GraphNode *g = *it;
    delete g;
...
llvm::DenseMap<GraphNode *, bool> taintedMap; // Para estatísticas de quantas
// arestas do grafo original estão
// em pelo menos 1 grafo tainted
// gerado por generateSubgraph()

int Graph::getTaintedEdges()
{
  int countEdges = 0;

  for (llvm::DenseMap<GraphNode *, bool>::iterator it = taintedMap.begin();
       it != taintedMap.end(); ++it)
  {
...
void Graph::findTaintedNodesSrcScanf(std::vector<Value *> src,
                                     std::vector<Value *> dst)
{
  ...
  for (std::set<GraphNode *>::iterator it = visitedNodes1.begin();
       it != visitedNodes1.end(); ++it)
  {
...
void Graph::findTaintedNodesMem(std::vector<Value *> src,
                                std::vector<Value *> dst)
{
  std::set<GraphNode *> visitedNodes1;
  std::set<GraphNode *> visitedNodes2;
...
Graph Graph::generateSubGraphMem(Value *src, Value *dst)
{
  ...
  for (std::map<GraphNode *, GraphNode *>::iterator it = nodeMap.begin();
       it != nodeMap.end(); ++it)
  {
    std::map<GraphNode *, edgeType> succs = it->first->getSuccessors();
    ...
    if (!G.nodes.count(it->second))
    {
      ...
      {
        G.varNodes[dyn_cast<VarNode>(it->second)->getValue()] =
            dyn_cast<VarNode>(it->second);
      }

      if (isa<MemNode>(it->second))
      {
        G.memNodes[dyn_cast<MemNode>(it->second)->getAliasSetId()] =
            dyn_cast<MemNode>(it->second);
      }

      if (isa<OpNode>(it->second))
      {
        G.opNodes[dyn_cast<OpNode>(it->second)->getValue()] =
            dyn_cast<OpNode>(it->second);

        if (isa<CallNode>(it->second))
        {
          G.callNodes[dyn_cast<CallNode>(it->second)->getCallInst()] =
              dyn_cast<CallNode>(it->second);
        }
      }
    }
  }
...
void Graph::dfsVisit(GraphNode *u, GraphNode *u2,
                     std::set<GraphNode *> &visitedNodes)
{
  // errs()<<u->getLabel()<<"\n";
...
void Graph::dfsVisitBack(GraphNode *u, GraphNode *u2,
                         std::set<GraphNode *> &visitedNodes)
{
  // errs()<<u->getLabel()<<"\n";
  visitedNodes.insert(u);
...
void Graph::toDotLinesPruned(std::string s, raw_ostream *stream)
{
  ...
  for (std::set<GraphNode *>::iterator node = nodes.begin(), end = nodes.end();
       node != end; node++)
  {
    label.str("");
         ...
         succ != s_end; succ++)
    {
      ...
      if (DefinedNodes.count(succ->first) == 0)
      {
        if (succ->first->shortestPath == true)
        {
          (*stream) << (succ->first)->getName()
                    ...
                    << ",style=" << (succ->first)->getStyle() << ",label=\""
                    << label.str() << "\", color=red]\n";
        }
        else
        {
      ...
      {
        if ((*node)->shortestPath && succ->first->shortestPath &&
            succ->first->predShortPath == (*node))
        {
...
// Print only shortest path
void Graph::toASCIILinesPruned(Value *e, std::string s,
...
void Graph::toASCIILinesPruned(Value *e, std::string s, raw_ostream *stream)
{
  ...
  if (nodeEnd != NULL)
  {
    ...
    while ((*node)->predShortPath != (*node))
    {
      ...
      if (DefinedNodes.count(*node) == 0 && label.str().compare("null") != 0 &&
          !label.str().empty())
      {
        if ((*node)->shortestPath == true)
        {
  ...
  /*
                  while (!invNodes.empty()) {
                          ostringstream *temp =
     cast<ostringstream*>(invNodes.pop());
                          (*stream) << temp->str() << "\n";
                  } */
}
...
void Graph::toDot(std::string s, raw_ostream *stream)
{
       ...
       node != end; node++)
  {
         ...
         succ != s_end; succ++)
    {
      ...
      {
        (*stream) << (succ->first)->getName()
                  << "[shape=" << (succ->first)->getShape() << ",style=filled"
                  << ",fillcolor=white"
...
void Graph::toDotSCCs(std::string s, std::string fileName)
{
  std::error_code EC;
  raw_fd_ostream File(fileName, EC, sys::fs::F_Text);
  ...
  this->toDotSCCs(s, &File, false);
}

void Graph::toDotOnlySCCs(std::string s, std::string fileName)
{
  std::error_code EC;
  raw_fd_ostream File(fileName, EC, sys::fs::F_Text);
  ...
  this->toDotSCCs(s, &File, true);
}

void Graph::toDotPrunned(std::string s, std::string fileName,
                         std::map<Value *, unsigned int> &instrToLoop)
{
  std::error_code EC;
  raw_fd_ostream File(fileName, EC, sys::fs::F_Text);
  ...
  this->toDotPrunned(s, &File, instrToLoop);
}

void Graph::getDirectedSubGraph(std::set<GraphNode *> *subgraph,
                                GraphNode *node)
{
  std::map<GraphNode *, edgeType> succs = node->getSuccessors();
  for (std::map<GraphNode *, edgeType>::iterator it = succs.begin();
       it != succs.end(); ++it)
  {
    // if (!dyn_cast<MemNode>(it->first))
    // {
      if (subgraph->find(it->first) == subgraph->end())
      {
        subgraph->insert(it->first);
        getDirectedSubGraph(subgraph, it->first);
      }
    // }
  }
...
void Graph::toDotSCCs(std::string s, raw_ostream *stream, bool only_scc = false)
{
  std::set<std::set<GraphNode *>> sccs;
  sccs = this->findStrongConnectedComponents();

  std::set<GraphNode *> q;
  GraphNode *node;

  // errs() << "Dependence Graph has " << sccs.size() << " SCC's!";
  // errs() << "\nPrinting the components more than size 1...\n\n";
  // for (std::set<std::set<GraphNode*> >::iterator it = sccs.begin();
  //     it != sccs.end(); ++it)
  // {
  //     if (it->size() > 1)
  //     {
  //         for (std::set<GraphNode*>::iterator s = it->begin(); s !=
  //         it->end();
  //             ++s)
  //         {
  //             errs() << " " << (*s)->getLabel();
  ...
  // Here print the graph with the components
  (*stream) << "digraph \"DFG for \'" << s << "\'  \"{\n";
  (*stream) << "label=\"DFG for \'" << s << "\' \";\n";
  ...
  if (only_scc)
  {
    std::set<GraphNode *> onlySccNodes;
    std::map<GraphNode *, edgeType> succs;
         ...
         it != sccs.end(); ++it)
    {
      if (it->size() > 2)
      {
        for (std::set<GraphNode *>::iterator s = it->begin(); s != it->end();
             ++s)
        {
          if (!dyn_cast<MemNode>(*s))
          {
            onlySccNodes.insert(*s);
            getDirectedSubGraph(&onlySccNodes, (*s));
          }
        }
      }
    }

    for (std::set<GraphNode *>::iterator s = onlySccNodes.begin();
         s != onlySccNodes.end(); ++s)
    {
      if (DefinedNodes.count(*s) == 0)
      {
        (*stream) << (*s)->getName() << "[shape=" << (*s)->getShape()
                  << ",style=filled"
                  << ",fillcolor=white"
                  << ",label=\"" << (*s)->getLabel() << "\"]\n";
        DefinedNodes[*s] = 1;
  ...
  {
    for (std::set<GraphNode *>::iterator node = nodes.begin(),
                                         end = nodes.end();
         node != end; node++)
    {
      if (DefinedNodes.count(*node) == 0)
...
void Graph::toDotPrunned(std::string s, raw_ostream *stream,
                         std::map<Value *, unsigned int> &instrToLoop)
{
  auto sccs = this->findStrongConnectedComponents();

  std::set<GraphNode *> q;
  GraphNode *node;

  string colors[] = {"white", "blue", "green", "orange", "red", "gray"};

  // Here print the graph with the components
  (*stream) << "digraph \"DFG for \'" << s << "\'  \"{\n";
  (*stream) << "label=\"DFG for \'" << s << "\' \";\n";

  // This dump the content of each SCC inside a DOT subgraph
  int i = 0;
  for (std::set<std::set<GraphNode *>>::iterator it = sccs.begin();
       it != sccs.end(); ++it)
  {
    ...
    /// Get pointer to current SCC and check if it has more than one node inside
    /// it
    q = (*it);
    if (q.size() > 2)
    {
      ...
      do
      {
        ...
        if (node->getValue() == nullptr)
          continue;
        if (instrToLoop[node->getValue()] == 0)
          continue;
  ...
  std::map<GraphNode *, int> AlreadyPrinted;
       ...
       node != end; node++)
  {
    ...
    if (instrToLoop[(*node)->getValue()] == 0)
      continue;
    auto loopId = instrToLoop[(*node)->getValue()];

    if (AlreadyPrinted.count(*node) == 0)
    {
      (*stream) << (*node)->getName() << "[shape=" << (*node)->getShape()
                << ",style=filled"
                << ",fillcolor=" << colors[loopId] << ",label=\""
                << (*node)->getLabel() << "\"]\n";
      AlreadyPrinted[*node] = 1;
    }
    std::map<GraphNode *, edgeType> succs = (*node)->getSuccessors();
    ...
    for (std::map<GraphNode *, edgeType>::iterator succ = succs.begin(),
         ...
         succ != s_end; succ++)
    {
      if (succ->first->getValue() == nullptr)
        continue;
      if (instrToLoop[succ->first->getValue()] == 0)
        continue;
      auto loopId = instrToLoop[(*node)->getValue()];

      if (AlreadyPrinted.count(succ->first) == 0)
      {
        (*stream) << (succ->first)->getName()
                  << "[shape=" << (succ->first)->getShape() << ",style=filled"
                  << ",fillcolor=" << colors[loopId] << ",label=\""
                  << (succ->first)->getLabel() << "\"]\n";
        AlreadyPrinted[succ->first] = 1;
      }
...
std::set<std::set<GraphNode *>> Graph::findStrongConnectedComponents()
{
	Value *val;
	bool gep;
       ...
       node != end; node++)
  {
    ...
    {
	  	val =(*node)->getValue();
		  if (val != nullptr && dyn_cast<GetElementPtrInst>(val))
		  {
		  	gep = true;
		  }
  	  if (!gep)
...
int Graph::tarjanVisit(GraphNode *v,
                       std::map<GraphNode *, std::pair<int, int>> *indexLowLink,
                       std::stack<GraphNode *> *stack,
                       std::map<GraphNode *, bool> *onStack,
...
void Graph::toDotColored(std::string s, std::string fileName)
{
  std::error_code EC;
  raw_fd_ostream File(fileName, EC, sys::fs::F_Text);
  ...
  this->toDotColored(s, &File);
}

void Graph::toDotColored(std::string s, raw_ostream *stream)
{
  (*stream) << "digraph \"DFG for \'" << s << "\'  \"{\n";
  (*stream) << "label=\"DFG for \'" << s << "\' \";\n";
  std::map<GraphNode *, int> DefinedNodes;
...
void llvm::Graph::toDot(std::string s, raw_ostream *stream,
                        llvm::Graph::Guider *g)
{
...
// Return false if some instruction inside the function f is a predecessor
// include in the map
bool Graph::check0InDegreeFunc(Function *f,
                               std::map<GraphNode *, edgeType> predecessors)
{
  for (Function::iterator Fit = f->begin(), Fend = f->end(); Fit != Fend; ++Fit)
  {
    for (BasicBlock::iterator bBIt = Fit->begin(), bBEnd = Fit->end();
         bBIt != bBEnd; ++bBIt)
    {
  ...
  if (findJoinNode(f) == NULL)
  {
         ...
         ++Fit)
    {
           ...
           bBIt != bBEnd; ++bBIt)
      {
        ...
        if (instNode != NULL)
        {
          if (check0InDegreeFunc(f, instNode->getPredecessors())) // Make edges
                                                                  // just for
                                                                  // nodes with
                                                                  // in-degree
                                                                  // == 0
...
GraphNode *Graph::addInst(Value *v)
{
  ...
  if (isValidInst(v))
  { // If is a data manipulator instruction
    Var = this->findNode(v);

    /*
     * If Var is NULL, the value hasn't been processed yet, so we must process
     * it
     *
     * However, if Var is a Pointer, maybe the memory node already exists but
     * the
     * operation node aren't in the graph, yet. Thus we must process it.
     */
    if (Var == NULL || (Var != NULL && findOpNode(v) == NULL))
    { // If it has not processed yet
      ...
      if (Var == NULL)
      {
        ...
        if (hasVarNode)
        {
          if (StoreInst *SI = dyn_cast<StoreInst>(v))
            Var = addInst(SI->getOperand(1)); // We do this here because we want
                                              // to represent the store
                                              // instructions as a flow of
                                              // information of a data to a
                                              // memory node
      ...
      if (isa<Instruction>(v))
      {
        ...
        for (unsigned int i = 0; i < cast<User>(v)->getNumOperands(); i++)
        {
          if (isa<StoreInst>(v) && i == 1)
            continue; // We do this here because we want to represent the store
                      // instructions as a flow of information of a data to a
                      // memory node
...
// It verifies if the instruction is valid for the dependence graph, i.e. just
// data manipulator instructions are important for dependence graph
...
std::set<GraphNode *> Graph::findNodes(std::set<Value *> values)
{
  ...
  for (std::set<Value *>::iterator i = values.begin(), end = values.end();
       i != end; i++)
  {
...
llvm::DenseMap<Value *, GraphNode *> llvm::Graph::getVarNodes()
{
  return varNodes;
}
...
std::pair<GraphNode *, int> llvm::Graph::getNearestDependency(
    llvm::Value *sink, std::set<llvm::Value *> sources, bool skipMemoryNodes)
{
  ...
  if (GraphNode *startNode = findNode(sink))
  {
    ...
    for (std::set<GraphNode *>::iterator Nit = nodes.begin(),
                                         Nend = nodes.end();
         Nit != Nend; Nit++)
    {
    ...
    while (workList.size())
    {
      ...
      for (std::map<GraphNode *, edgeType>::iterator pred = preds.begin(),
           ...
           pred != pend; pred++)
      {
        if (nodeColor[pred->first] == 0)
        { // the node hasn't been processed yet

          nodeColor[pred->first] = 1;

          workList.push_back(
              pair<GraphNode *, int>(pred->first, currentDistance + 1));
        }
      }
    }
  }
  ...
  if (GraphNode *startNode = findNode(sink))
  {
    ...
    while (!workList.empty())
    {
      ...
      if (sourceNodes.count(workNode))
      {
        ...
        //                                path.end());
        //                              errs() << "Path: ";
        //                              for (std::vector<GraphNode*>::iterator i
        //                              = path.begin(), e = path.end(); i != e;
        //                              ++i) {
        //                                      errs() << (*i)->getLabel() << "
        //                                      | ";
        //                              }
...
// Project: e-CoSoc (Intel and Computer Science department of Federal University
// of Minas Gerais)
//
//*********************************************************************************************************************************************************************
...
bool FunctionDepGraph::runOnFunction(Function &F)
{
  ...
  for (Function::iterator BBit = F.begin(), BBend = F.end(); BBit != BBend;
       ++BBit)
  {
    for (BasicBlock::iterator Iit = BBit->begin(), Iend = BBit->end();
         Iit != Iend; ++Iit)
    {
  ...
  // {
  //     for (std::set<GraphNode*>::iterator it = nodes.begin();
  //         it != nodes.end(); ++it)
  //     {
  //         if (!dyn_cast<MemNode>(*it) && ((V = (*it)->getValue())))
  //         {
  //             originalName = RN->getNameofValue(V).nameInFile;
  //             if (!originalName.empty())
  //             {
  //                 errs() << "Name " << (*it)->getLabel() << ": " <<
  //                 originalName << "\n";
  //                 (*it)->setOriginalName(originalName);
  //             }
  //         }
  //     }
  // }
...
bool ModuleDepGraph::runOnModule(Module &M)
{
  ...
  // Insert instructions in the graphs
  for (Module::iterator Fit = M.begin(), Fend = M.end(); Fit != Fend; ++Fit)
  {
    for (Function::iterator BBit = Fit->begin(), BBend = Fit->end();
         BBit != BBend; ++BBit)
    {
      for (BasicBlock::iterator Iit = BBit->begin(), Iend = BBit->end();
           Iit != Iend; ++Iit)
  ...
  // Here: recover the original names and set them
  std::set<GraphNode *> nodes = depGraph->getNodes();
  ...
  for (Module::iterator Fit = M.begin(), Fend = M.end(); Fit != Fend; ++Fit)
  {
    ...
    // Empty functions include externally linked ones (i.e. abort, printf,
    // scanf, ...)
  ...
  // for (Module::iterator Fit = M.begin(), Fend = M.end(); Fit != Fend; ++Fit)
  // {
  //     if (!Fit->getBasicBlockList().empty())
  //     {
  //         RN = &(getAnalysis<RecoverNames> (*Fit));
  //         for (std::set<GraphNode*>::iterator it = nodes.begin();
  //             it != nodes.end(); ++it)
  //         {
  //             if (!dyn_cast<MemNode>(*it) && ((V = (*it)->getValue())))
  //             {
  //                 RecoverNames::VarNames originalVar = RN->getNameofValue(V);
  //                 if (originalVar != NULL)
  //                 {
  //                     originalName = originalVar.nameInFile;
  //                     if (!originalName.empty())
  //                     {
  //                         errs() << "Name " << (*it)->getLabel() << ": " <<
  //                         originalName << "\n";
  ...
  // //printing var Original names
  // depGraph->printVarNodes();
...
llvm::Graph::Guider::Guider(Graph *graph)
{
  ...
  for (std::set<GraphNode *>::iterator i = nodes.begin(), e = nodes.end();
       i != e; ++i)
  {
    nodeAttrs[*i] = "[label=\"" + (*i)->getLabel() + "\" shape=\"" +
                    (*i)->getShape() + "\" style=\"" + (*i)->getStyle() + "\"]";
  }
}
...
char ViewFunctionDepGraph::ID = 0;
static RegisterPass<ViewFunctionDepGraph> W("view-funcdepgraph",
                                          "View Function Dependence Graph");
